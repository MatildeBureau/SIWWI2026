...
% has_ice_p2: true for ice runs, false for open-water benchmark
has_ice_p2 = false;   % <-- SET PER RUN
...
for i = 1:numFiles
    ...
    fprintf('  [PARSE CHECK] fFreq raw = %.6f Hz, fAmp raw = %.6f V\n', fFreq, fAmp);
...
%% TRIM RESULT ARRAYS
n_rows = kOut - 1;   % safe even if loop aborted early
...
fprintf('\n[DIAG] Stored (InputFreq, InputAmp) pairs in result arrays:\n');
if ~isempty(idx_acou)
    stored_pairs = unique([res_InputFreq(idx_acou), res_InputAmp(idx_acou)], 'rows');
    for ip = 1:size(stored_pairs,1)
        fprintf('  f=%.4f Hz  V=%.4f V\n', stored_pairs(ip,1), stored_pairs(ip,2));
    end
end
...
if hasBenchmark
    ...
    if skipSensor5 
        plotNormSuffix = '_NORM_s5skipped';
    end

else
    
    plotNormSuffix = '_s5skipped';
end
...
% ---------------------------------------------------------
%  FALLBACK: ensure ka_meas/dka_meas/ka_set columns always exist,
%  even if Plot 2's guards skip the broadcast step (Step 1c).
%  Overwritten by Plot 2 if it runs successfully.
% ---------------------------------------------------------
res_ka_meas  = NaN(n_rows, 1);
res_dka_meas = NaN(n_rows, 1);
res_ka_set   = NaN(n_rows, 1);

% %% =========================================================
% %  PLOT 2 (REVISED v3): AMPLITUDE vs POSITION — DOLATSHAH GRID
% %
% %  Grid:  ROWS = unique T_set (period varies down rows, ka_set fixed
% %                across a row would be one *column*... see below)
% %         COLS = unique ka_set
% %
% %  Following Dolatshah layout:
% %    - columns = fixed (ka_set, varying T_set down the column)... 
% %    Actually Dolatshah's layout (and the image you attached) is:
% %      ROWS    = fixed (ka_set, T_set) per subplot
% %      one subplot per UNIQUE (ka_set, T_set) PAIR directly from metadata
% %      arranged so that going ACROSS a row, T_set is fixed and ka_set varies
% %      going DOWN a column, ka_set is fixed and T_set varies
% %
% %  Each subplot = ONE (ka_set, T_set) condition = ONE (ampC, freqC) pair.
% %  No more multiple overlapping series per subplot.
% %
% %  Subplot title:  (ka_set, T_set)  in front,  (ka_meas, T_meas) in
% %  brackets — bracket part omitted if uncertainty too small to matter
% %  or if ka_set is NaN (no metadata match).
% %
% %  Amplitude normalised by reference if hasBenchmark, else raw [m].
% % =========================================================
% 
% % ---------------------------------------------------------
% %  USER PARAMETERS
% % ---------------------------------------------------------
% f_tol_p2       = 0.01;    % [Hz] half-width for frequency bin matching
% volt_tol_p2    = 0.02;    % [V]  voltage tolerance for metadata lookup
% skipSensor5_p2 = false;
% sensor5_loc_p2 = 7.80;    % [m]
% col_acou_p2    = [0.18 0.45 0.72];
% col_cam_p2     = [0.80 0.20 0.10];
% 
% % Minimum displayable uncertainty (below this, omit +/- from annotation)
% ka_unc_display_tol = 1e-4;   % [-]
% T_unc_display_tol  = 1e-3;   % [s]
% 
% % ---------------------------------------------------------
% %  ICE PARAMETERS  (mirrored from wave_class.m)
% % ---------------------------------------------------------
% E_ice_p2   = 2.1e9;   % [Pa]
% h_ice_p2   = 0.01;    % [m]
% nu_ice_p2  = 0.33;    % [-]
% rho_w_p2   = 1000;    % [kg/m^3]
% g_p2       = 9.81;    % [m/s^2]
% 
% dE_ice_p2  = 0.2 * E_ice_p2;   % [Pa]
% dh_ice_p2  = 0.0005;           % [m]
% 
% D_flex_p2  = E_ice_p2 * h_ice_p2^3 / (12*(1 - nu_ice_p2^2));
% relErr_D_flex_p2 = sqrt( (dE_ice_p2/E_ice_p2)^2 + (3 * dh_ice_p2/h_ice_p2)^2 );
% dD_flex_p2       = D_flex_p2 * relErr_D_flex_p2;
% 
% fprintf('  [Plot2] D_flex = %.4e +/- %.4e N.m  (%.1f%% rel. unc.)\n', ...
%     D_flex_p2, dD_flex_p2, relErr_D_flex_p2*100);
% 
% % ---------------------------------------------------------
% %  STEP 1 — BUILD CONDITION TABLE: one row per unique (Aset, fset)
% %  found in the ACOUSTIC results, with ka_set/T_set looked up from
% %  pairMeta and ka_meas/dka_meas computed from the dispersion relation.
% % ---------------------------------------------------------
% if isempty(idx_acou)
%     fprintf('  [SKIP Plot2] No acoustic data.\n');
% else
% % --- BULLETPROOF COLUMN MATCHING ---
% pVars = pairMeta.Properties.VariableNames;
% 
% % Find columns by looking for parts of the name (ignoring case and exact formatting)
% idx_ka = find(strcmpi(pVars, 'ka') | contains(pVars, 'ka_', 'IgnoreCase', true), 1);
% idx_f  = find(contains(pVars, 'f_Hz', 'IgnoreCase', true), 1);
% idx_v  = find(contains(pVars, 'volt_V', 'IgnoreCase', true), 1);
% 
% has_ka_col = ~isempty(idx_ka);
% has_f_col  = ~isempty(idx_f);
% has_v_col  = ~isempty(idx_v);
% 
% % Force the names to match exactly what the rest of the script expects
% if has_ka_col, pairMeta.Properties.VariableNames{idx_ka} = 'ka'; end
% if has_f_col, pairMeta.Properties.VariableNames{idx_f} = 'Set_f_Hz'; end
% if has_v_col, pairMeta.Properties.VariableNames{idx_v} = 'Set_volt_V'; end
% 
% if ~(has_ka_col && has_f_col && has_v_col)
%     fprintf(2, '\n  [SKIP Plot2] STILL MISSING COLUMNS!\n');
%     fprintf(2, '  Here are the columns MATLAB actually loaded:\n');
%     disp(pVars');
% end
% 
% cond_pairs_p2 = unique([res_InputAmp(idx_acou), res_InputFreq(idx_acou)], 'rows');
% cond_pairs_p2 = cond_pairs_p2(all(isfinite(cond_pairs_p2),2), :);
% nCond_p2      = size(cond_pairs_p2, 1);
% 
% ka_set_p2    = NaN(nCond_p2, 1);   % [-]   set steepness from metadata
% T_set_p2     = NaN(nCond_p2, 1);   % [s]   set period (= 1/freqC)
% ka_meas_p2   = NaN(nCond_p2, 1);   % [-]   measured steepness (FFT amp at x_ref)
% dka_meas_p2  = NaN(nCond_p2, 1);   % [-]   uncertainty on ka_meas
% T_meas_p2    = NaN(nCond_p2, 1);   % [s]   measured period at x_ref
% dT_meas_p2   = NaN(nCond_p2, 1);   % [s]   uncertainty on T_meas
% k_cond_p2    = NaN(nCond_p2, 1);   % [m^-1]
% dk_cond_p2   = NaN(nCond_p2, 1);   % [m^-1]
% 
% x_ref_p2 = 4.6;   % [m]  reference sensor for ka_meas / T_meas estimate
% 
% for ic = 1:nCond_p2
%     ampC  = cond_pairs_p2(ic, 1);   % [V]
%     freqC = cond_pairs_p2(ic, 2);   % [Hz]
% 
%     T_set_p2(ic) = 1 / freqC;   % [s] nominal set period
% 
%     % --- Dispersion: k for this set frequency (central value) ---
%     omega_ic = 2*pi*freqC;
%     k_ic     = deal_solve_k(omega_ic, water_depth_m, D_flex_p2, rho_w_p2, g_p2, has_ice_p2);
%     k_cond_p2(ic) = k_ic;
% 
%     if has_ice_p2 && dD_flex_p2 > 0
%         k_plus  = deal_solve_k(omega_ic, water_depth_m, D_flex_p2 + dD_flex_p2, rho_w_p2, g_p2, has_ice_p2);
%         k_minus = deal_solve_k(omega_ic, water_depth_m, D_flex_p2 - dD_flex_p2, rho_w_p2, g_p2, has_ice_p2);
%         dk_cond_p2(ic) = abs(k_plus - k_minus) / 2;   % [m^-1]
%     else
%         dk_cond_p2(ic) = 0;
%     end
% 
%     % --- ka_set from metadata: match (Set_f_Hz, Set_volt_V) ---
%     pm_f  = pairMeta.Set_f_Hz;
%     pm_v  = pairMeta.Set_volt_V;
% 
%     if has_ka_col
%         pm_ka = pairMeta.ka;
%     else
%         pm_ka = NaN(height(pairMeta), 1); % Create a dummy array of NaNs if the column is missing
%     end
% 
% 
%     meta_match = find( ...
%         abs(pm_f - freqC) < f_tol_p2 & ...
%         abs(pm_v - ampC)  < volt_tol_p2 & ...
%         isfinite(pm_ka));
% 
%     if ~isempty(meta_match)
%         ka_set_p2(ic) = pm_ka(meta_match(1));
%     else
%         fprintf('  [WARN Plot2] No ka found in pairMeta for f=%.4fHz V=%.4fV (ka_set -> NaN)\n', freqC, ampC);
%         ka_set_p2(ic) = NaN;
%     end
% 
%     % --- ka_meas / T_meas at x_ref ---
%     mask_ref = idx_acou( ...
%         abs(res_InputAmp(idx_acou)  - ampC)  < 1e-6 & ...
%         abs(res_InputFreq(idx_acou) - freqC) < 1e-6 & ...
%         abs(res_SensorLoc(idx_acou) - x_ref_p2) < 0.15 & ...
%         isfinite(res_MeanAmp(idx_acou)));
% 
%     if ~isempty(mask_ref)
%         a_ref_ic  = mean(res_MeanAmp(mask_ref));      % [m]
%         da_ref_ic = mean(res_UncAmp(mask_ref));       % [m]
% 
%         ka_meas_p2(ic)  = k_ic * a_ref_ic;
%         dka_meas_p2(ic) = sqrt( (a_ref_ic * dk_cond_p2(ic))^2 ...
%                                + (k_ic     * da_ref_ic)^2 );
% 
%         % T_meas from FFT frequency at the same rows
%         f_meas_ic  = mean(res_FFT_Freq(mask_ref), 'omitnan');     % [Hz]
%         df_meas_ic = mean(res_UncFreq(mask_ref),  'omitnan');     % [Hz]
%         if isfinite(f_meas_ic) && f_meas_ic > 0
%             T_meas_p2(ic)  = 1 / f_meas_ic;                       % [s]
%             % dT = |dT/df| * df = (1/f^2) * df
%             dT_meas_p2(ic) = (df_meas_ic / f_meas_ic^2);          % [s]
%         end
%     end
% end
% 
% % ---------------------------------------------------------
% %  STEP 1c — BROADCAST ka_meas / dka_meas / ka_set BACK TO
% %  EVERY RESULT ROW, matched by (Aset, fset)
% % ---------------------------------------------------------
% res_ka_meas  = NaN(n_rows, 1);
% res_dka_meas = NaN(n_rows, 1);
% res_ka_set   = NaN(n_rows, 1);
% 
% for ic = 1:nCond_p2
%     ampC  = cond_pairs_p2(ic, 1);
%     freqC = cond_pairs_p2(ic, 2);
%     rowMask = abs(res_InputAmp  - ampC)  < 1e-6 & ...
%               abs(res_InputFreq - freqC) < 1e-6;
%     res_ka_meas(rowMask)  = ka_meas_p2(ic);
%     res_dka_meas(rowMask) = dka_meas_p2(ic);
%     res_ka_set(rowMask)   = ka_set_p2(ic);
% end
% 
% fprintf('  [Plot2] ka_meas/ka_set broadcast to %d / %d result rows.\n', ...
%     sum(~isnan(res_ka_meas)), n_rows);
% 
% % ---------------------------------------------------------
% %  STEP 2 — DEFINE SUBPLOT GRID
% %  ROWS = unique T_set values (ascending)
% %  COLS = unique ka_set values (ascending), NaN ka_set excluded from
% %         grid axes but its condition is still plotted if it matches
% %         a (NaN-row is impossible since ka_set defines the column —
% %         conditions with NaN ka_set are skipped from the grid entirely,
% %         consistent with "if ka_set is absent ... don't display it")
% % ---------------------------------------------------------
% valid_grid = isfinite(ka_set_p2) & isfinite(T_set_p2);
% 
% unique_ka_p2 = unique(ka_set_p2(valid_grid));
% unique_T_p2  = unique(T_set_p2(valid_grid));
% 
% % Sort rows by T_set descending (longest period at top, like Dolatshah)
% unique_T_p2 = sort(unique_T_p2, 'descend');
% unique_ka_p2 = sort(unique_ka_p2, 'ascend');
% 
% nKA_p2 = length(unique_ka_p2);   % columns
% nT_p2  = length(unique_T_p2);    % rows
% 
% if nKA_p2 == 0 || nT_p2 == 0
%     fprintf('  [SKIP Plot2] Empty ka_set/T_set grid after metadata lookup.\n');
% else
% 
% % ---------------------------------------------------------
% %  STEP 3 — FIGURE AND SUBPLOTS
% %  rows = T_set (period), cols = ka_set (steepness)
% % ---------------------------------------------------------
% fAmpFig2 = figure('Name', sprintf('Plot2_Dolatshah%s', plotNormSuffix));
% fAmpFig2.Position = [80, 80, 210*nKA_p2 + 80, 185*nT_p2 + 100];
% 
% ax2grid_p2 = gobjects(nT_p2, nKA_p2);
% 
% for iT = 1:nT_p2
%     for iKA = 1:nKA_p2
%         T_tgt  = unique_T_p2(iT);
%         ka_tgt = unique_ka_p2(iKA);
% 
%         % Exactly ONE condition should match this (ka_set, T_set) pair
%         match_ic = find( ...
%             abs(ka_set_p2 - ka_tgt) < 1e-9 & ...
%             abs(T_set_p2  - T_tgt)  < 1e-9);
% 
%         spIdx  = (iT-1)*nKA_p2 + iKA;
%         ax_p2  = subplot(nT_p2, nKA_p2, spIdx);
%         ax2grid_p2(iT, iKA) = ax_p2;
%         hold(ax_p2,'on'); grid(ax_p2,'on'); box(ax_p2,'on');
% 
%         if isempty(match_ic)
%             text(ax_p2, 0.5, 0.5, 'no data', 'Units','normalized', ...
%                 'HorizontalAlignment','center','Color',[0.6 0.6 0.6], ...
%                 'FontSize',8,'Interpreter','none');
%         else
%             ic1 = match_ic(1);   % should be unique
% 
%             ka_meas_here  = ka_meas_p2(ic1);
%             dka_meas_here = dka_meas_p2(ic1);
%             T_meas_here   = T_meas_p2(ic1);
%             dT_meas_here  = dT_meas_p2(ic1);
% 
%             % ---- Build annotation: (ka_set, T_set)  (ka_meas, T_meas) ----
%             main_str = sprintf('($ka$, $T$)$_\\mathrm{set}$ = (%.3f, %.2f s)', ...
%                 ka_tgt, T_tgt);
% 
%             % Bracket part: only if ka_set was not NaN (it isn't, by
%             % construction here since valid_grid required isfinite) AND
%             % measured values exist
%             bracket_str = '';
%             if isfinite(ka_meas_here) && isfinite(T_meas_here)
%                 % ka_meas with/without uncertainty
%                 if isfinite(dka_meas_here) && dka_meas_here > ka_unc_display_tol
%                     ka_meas_fmt = sprintf('%.3f$\\pm$%.3f', ka_meas_here, dka_meas_here);
%                 else
%                     ka_meas_fmt = sprintf('%.3f', ka_meas_here);
%                 end
%                 % T_meas with/without uncertainty
%                 if isfinite(dT_meas_here) && dT_meas_here > T_unc_display_tol
%                     T_meas_fmt = sprintf('%.2f$\\pm$%.2f s', T_meas_here, dT_meas_here);
%                 else
%                     T_meas_fmt = sprintf('%.2f s', T_meas_here);
%                 end
%                 bracket_str = sprintf('\n($ka$, $T$)$_\\mathrm{meas}$ = (%s, %s)', ...
%                     ka_meas_fmt, T_meas_fmt);
%             end
% 
%             annot_str = [main_str, bracket_str];
% 
%             text(ax_p2, 0.97, 0.95, annot_str, ...
%                 'Units','normalized', ...
%                 'HorizontalAlignment','right','VerticalAlignment','top', ...
%                 'Interpreter','latex','FontSize',7.5);
% 
%             % ---- Plot the single matching condition ----
%             ampC2  = cond_pairs_p2(ic1, 1);
%             freqC2 = cond_pairs_p2(ic1, 2);
% 
%             idxA2 = idx_acou( ...
%                 abs(res_InputAmp(idx_acou)  - ampC2)  < 1e-6 & ...
%                 abs(res_InputFreq(idx_acou) - freqC2) < 1e-6 & ...
%                 isfinite(res_MeanAmp_plot(idx_acou)));
%             if skipSensor5_p2
%                 idxA2 = idxA2(abs(res_SensorLoc(idxA2) - sensor5_loc_p2) > 0.1);
%             end
% 
%             idxC2 = idx_cam( ...
%                 abs(res_InputAmp(idx_cam)  - ampC2)  < 1e-6 & ...
%                 abs(res_InputFreq(idx_cam) - freqC2) < 1e-6 & ...
%                 isfinite(res_MeanAmp_plot(idx_cam)));
%             if skipSensor5_p2
%                 idxC2 = idxC2(abs(res_SensorLoc(idxC2) - sensor5_loc_p2) > 0.1);
%             end
% 
%             for ii = idxA2'
%                 errorbar(ax_p2, ...
%                     res_SensorLoc(ii), res_MeanAmp_plot(ii), ...
%                     res_UncAmp_plot(ii), res_UncAmp_plot(ii), ...
%                     delta_x_m, delta_x_m, ...
%                     'x','Color',col_acou_p2,'MarkerFaceColor',col_acou_p2, ...
...
%                     res_UncAmp_plot(ii), res_UncAmp_plot(ii), ...
%                     delta_x_m, delta_x_m, ...
%                     'o','Color',col_cam_p2,'MarkerFaceColor','none', ...
%                     'MarkerSize',8,'CapSize',3,'LineWidth',1.2, ...
%                     'HandleVisibility','off');
%             end
% 
%             if hasBenchmark
%                 yline(ax_p2, 1,'k--','LineWidth',0.8,'HandleVisibility','off');
%             end
%         end % isempty guard
% 
%         % Axis decoration
%         xlim(ax_p2, [-0.5, 12]);
%         yl = ylim(ax_p2);
%         ylim(ax_p2, [0, max(yl(2), hasBenchmark*1.3 + ~hasBenchmark*0.005)]);
% 
%         if iT == nT_p2
%             xlabel(ax_p2,'$x$ (m)','Interpreter','latex','FontSize',9);
%         else
%             set(ax_p2,'XTickLabel',[]);
%         end
%         if iKA == 1
%             ylabel(ax_p2, y_label_amp,'Interpreter','latex','FontSize',9);
%         else
%             set(ax_p2,'YTickLabel',[]);
%         end
%         ax_p2.FontSize = 8;
% 
%     end % iKA
...
% %  SHARED LEGEND
% % ---------------------------------------------------------
% ax_leg = ax2grid_p2(nT_p2, nKA_p2);
% hA_leg = errorbar(ax_leg, NaN,NaN,NaN,NaN, ...
%     'x','Color',col_acou_p2,'MarkerFaceColor',col_acou_p2, ...
%     'MarkerSize',8,'CapSize',3,'LineWidth',1.2,'DisplayName','Acoustic sensor');
% hC_leg = errorbar(ax_leg, NaN,NaN,NaN,NaN, ...
%     'o','Color',col_cam_p2,'MarkerFaceColor','none', ...
%     'MarkerSize',8,'CapSize',3,'LineWidth',1.2,'DisplayName','Camera');
% lgd_p2 = legend(ax_leg,[hA_leg,hC_leg], ...
%     'Location','southoutside','Interpreter','latex', ...
%     'FontSize',8,'NumColumns',2,'Box','off');
% 
% if has_ice_p2
%     disp_note = sprintf('Ice — $D=(%.2e \\pm %.2e)$ N$\\cdot$m  ($h=%.0f\\pm%.2f$ mm, $E=%.1f\\pm%.1f$ GPa)', ...
%         D_flex_p2, dD_flex_p2, h_ice_p2*1e3, dh_ice_p2*1e3, ...
%         E_ice_p2*1e-9, dE_ice_p2*1e-9);
% else
%     disp_note = 'Open water';
% end
% annotation(fAmpFig2,'textbox',[0.01 0.001 0.98 0.03], ...
%     'String', disp_note, ...
%     'Interpreter','latex','FontSize',7,'HorizontalAlignment','center', ...
%     'VerticalAlignment','bottom','EdgeColor','none');
% 
% figHandles{end+1}   = fAmpFig2;
% figBaseNames{end+1} = sprintf('Plot2_Dolatshah%s', plotNormSuffix);
% 
% end % nKA/nT guard
% end % idx_acou guard
% 
...
% %% =========================================================
% %  PLOT 2 (REVISED v4): AMPLITUDE vs POSITION — DOLATSHAH GRID
% %
% %  ONE FIGURE PER SET STEEPNESS (ka_set).
% %  Within each figure: one row of subplots, one per T_set
% %  (period), sorted descending (longest period on the left).
% %
% %  Each subplot = ONE (ka_set, T_set) condition.
% %  Empty (ka_set, T_set) combinations are SKIPPED (no figure/subplot).
% %
% %  Annotation (ka, T)_set / (ka, T)_meas placed ABOVE each subplot
% %  (as a title, outside the axes).
% %
% %  Shared Acoustic/Camera legend placed below all subplots in each
% %  figure (replaces the open-water/ice annotation).
% %
% %  Square axes, larger markers/fonts throughout.
% %
% %  Amplitude normalised by reference if hasBenchmark, else raw [m].
% % =========================================================
% 
...
% col_cam_p2     = [0.80 0.20 0.10];
% 
% % Display precision (also used as the "last digit" uncertainty threshold)
% ka_decimals = 3;          % ka displayed to 3 dp -> last digit = 0.001
% T_decimals  = 2;          % T  displayed to 2 dp -> last digit = 0.01
% ka_unc_display_tol = 10^(-ka_decimals);   % [-]   = 0.001
% T_unc_display_tol  = 10^(-T_decimals);    % [s]   = 0.01
% 
% % Visual scaling (bigger markers/fonts/lines)
% mk_size_data   = 11;      % marker size for acoustic/camera data points
% lw_data        = 1.8;     % line width for data error bars
% cap_size_data  = 5;       % error bar cap size
% fs_axis        = 13;      % axis tick label font size
% fs_label       = 16;      % axis label font size
% fs_annot       = 12;      % per-subplot (ka,T) annotation font size
% fs_legend      = 13;      % legend font size
% mk_size_legend = 11;      % marker size in legend
% 
% % ---------------------------------------------------------
% %  ICE PARAMETERS  (mirrored from wave_class.m)
% % ---------------------------------------------------------
...
% 
% % ---------------------------------------------------------
% %  STEP 2 — DEFINE FIGURE/SUBPLOT GRID
% %  ONE FIGURE PER unique ka_set (ascending)
% %  WITHIN each figure: ONE ROW of subplots, ONE PER unique T_set
% %  that actually has data for this ka_set, sorted descending.
% %  Combinations with NO data are skipped entirely (no empty subplot).
% % ---------------------------------------------------------
% valid_grid = isfinite(ka_set_p2) & isfinite(T_set_p2);
% 
% unique_ka_p2 = sort(unique(ka_set_p2(valid_grid)), 'ascend');
% 
% if isempty(unique_ka_p2)
%     fprintf('  [SKIP Plot2] Empty ka_set/T_set grid after metadata lookup.\n');
% else
% 
% for iKAfig = 1:length(unique_ka_p2)
%     ka_tgt = unique_ka_p2(iKAfig);
% 
%     % T_set values that exist for THIS ka_set
%     T_here = T_set_p2(valid_grid & abs(ka_set_p2 - ka_tgt) < 1e-9);
%     T_here = sort(unique(T_here), 'descend');
% 
%     % Of these, keep only those with an actual matching condition
%     % (should always be true by construction, but double-check)
%     keepT = false(size(T_here));
%     matchIdxPerT = cell(size(T_here));
%     for iT = 1:length(T_here)
%         mi = find(abs(ka_set_p2 - ka_tgt) < 1e-9 & abs(T_set_p2 - T_here(iT)) < 1e-9);
%         if ~isempty(mi)
%             keepT(iT) = true;
%             matchIdxPerT{iT} = mi(1);
%         end
%     end
%     T_here       = T_here(keepT);
%     matchIdxPerT = matchIdxPerT(keepT);
% 
%     if isempty(T_here)
%         fprintf('  [SKIP Plot2] No valid T_set conditions for ka_set=%.3f — figure skipped.\n', ka_tgt);
%         continue;
%     end
% 
%     nT_this = length(T_here);
% 
%     % ---------------------------------------------------------
%     %  FIGURE: one row of nT_this subplots
%     % ---------------------------------------------------------
%     figName = sprintf('Plot2_ka%s%s', ...
%         strrep(sprintf('%.3f', ka_tgt), '.', 'p'), plotNormSuffix);
%     fAmpFig2 = figure('Name', figName);
%     fAmpFig2.Position = [60, 60, 420*nT_this + 100, 480]; 
% 
%     ax2_this = gobjects(1, nT_this);
% 
%     for iT = 1:nT_this
%         T_tgt = T_here(iT);
%         ic1   = matchIdxPerT{iT};
% 
%         ax_p2 = subplot(1, nT_this, iT);
%         ax2_this(iT) = ax_p2;
%         hold(ax_p2,'on'); grid(ax_p2,'on'); box(ax_p2,'on');
%         pbaspect(ax_p2, [1 1 1]);   % square axes
%         axis(ax_p2, 'square');
% 
%         ka_meas_here  = ka_meas_p2(ic1);
%         dka_meas_here = dka_meas_p2(ic1);
...
%         dT_meas_here  = dT_meas_p2(ic1);
% 
%         % ---- Build (ka,T)_set / (ka,T)_meas annotation ----
%         % main_str = sprintf('($ka$, $T$)$_\\mathrm{set}$ = (%.2f, %.1f s)', ...
%         %     ka_tgt, T_tgt);
%         main_str = sprintf('$T_\\mathrm{set}$ =  %.1f s', ...
%              T_tgt);
% 
%         bracket_str = '';
%         % if isfinite(ka_meas_here) && isfinite(T_meas_here)
...
%         annot_str = [main_str, bracket_str];
% 
%         % Placed above the axes via title 
%         title(ax_p2, annot_str, 'Interpreter','latex', 'FontSize', fs_annot, ...
%             'FontWeight','normal');
% 
%         % ---- Plot the single matching condition ----
%         ampC2  = cond_pairs_p2(ic1, 1);
...
%                 delta_x_m, delta_x_m, ...
%                 'x','Color',col_acou_p2,'MarkerFaceColor',col_acou_p2, ...
%                 'MarkerSize',mk_size_data,'CapSize',cap_size_data,'LineWidth',lw_data, ...
%                 'HandleVisibility','off');
%         end
% 
%         for ii = idxC2'
...
% 
%         if hasBenchmark
%             yline(ax_p2, 1,'k--','LineWidth',1.2,'HandleVisibility','off');
%         end
% 
%         % % Axis decoration
%         % xlim(ax_p2, [-0.5, 12]);
...
% 
%         % Axis decoration
%         xlim(ax_p2, [-1.0, 12.5]); % Expanded left/right margins
%         yl = ylim(ax_p2);
% 
%         % Multiply yl(2) by 1.15 to add a 15% buffer at the top of the plot
%         ylim(ax_p2, [0, max(yl(2)*1.15, hasBenchmark*1.3 + ~hasBenchmark*0.005)]);
% 
%         xlabel(ax_p2,'$x$ (m)','Interpreter','latex','FontSize',fs_label);
%         if iT == 1
%             ylabel(ax_p2, y_label_amp,'Interpreter','latex','FontSize',fs_label);
%         else
%             set(ax_p2,'YTickLabel',[]);
%         end
%         ax_p2.FontSize = fs_axis;
%         ax_p2.LineWidth = 1.2;
% 
%     end % iT
% 
%     % % ---------------------------------------------------------
%     % %  SHARED LEGEND (Acoustic / Camera) BELOW ALL SUBPLOTS
%     % %  Replaces the open-water/ice annotation entirely.
%     % % ---------------------------------------------------------
%     % % Use an invisible axes spanning the figure for the legend
%     % legAx = axes(fAmpFig2, 'Position', [0 0 1 0.001], 'Visible', 'off');
%     % hold(legAx, 'on');
%     % % Give the legend a title using object properties instead of the title() function
%     % lgd_p2.Title.String      = sprintf('$(ka)_\\mathrm{set} = %.2f$', ka_tgt);
%     % lgd_p2.Title.Interpreter = 'latex';
%     % lgd_p2.Title.FontSize    = fs_legend;
%     % lgd_p2.Title.FontWeight  = 'normal';
%     % hA_leg = errorbar(legAx, NaN,NaN,NaN,NaN, ...
%     %     'x','Color',col_acou_p2,'MarkerFaceColor',col_acou_p2, ...
%     %     'MarkerSize',mk_size_legend,'CapSize',cap_size_data,'LineWidth',lw_data, ...
%     %     'DisplayName','Acoustic sensor');
%     % hC_leg = errorbar(legAx, NaN,NaN,NaN,NaN, ...
%     %     'o','Color',col_cam_p2,'MarkerFaceColor','none', ...
%     %     'MarkerSize',mk_size_legend,'CapSize',cap_size_data,'LineWidth',lw_data, ...
%     %     'DisplayName','Camera');
%     % lgd_p2 = legend(legAx,[hA_leg,hC_leg], ...
%     %     'Location','south','Interpreter','latex', ...
%     %     'FontSize',fs_legend,'NumColumns',2,'Box','off', ...
%     %     'Orientation','horizontal');
%     % lgd_p2.Position(2) = 0.05;   % pin near bottom edge
%     % 
%     % 
%     % 
%     % % Tighten layout: leave room at top for per-subplot titles and
%     % % bottom for the shared legend
%     % for iT = 1:nT_this
%     %     p = get(ax2_this(iT), 'Position');
%     %     ax2_this(iT).Position = [p(1), p(2)+0.10, p(3), p(4)-0.05];
%     % end
%     % ---------------------------------------------------------
%     %  SHARED LEGEND (Acoustic / Camera / Text) BELOW ALL SUBPLOTS
%     % ---------------------------------------------------------
%     % Use an invisible axes spanning the figure for the legend
%     legAx = axes(fAmpFig2, 'Position', [0 0 1 0.001], 'Visible', 'off');
%     hold(legAx, 'on');
% 
%     % 1. Dummy invisible plot just to hold the ka_set text in the legend
%     ka_str = sprintf('$(ka)_\\mathrm{set} = %.2f$', ka_tgt);
%     hText_leg = plot(legAx, NaN, NaN, 'LineStyle', 'none', 'Marker', 'none', ...
%         'DisplayName', ka_str);
% 
%     % 2. Acoustic dummy plot
%     hA_leg = errorbar(legAx, NaN,NaN,NaN,NaN, ...
%         'x','Color',col_acou_p2,'MarkerFaceColor',col_acou_p2, ...
%         'MarkerSize',mk_size_legend,'CapSize',cap_size_data,'LineWidth',lw_data, ...
%         'DisplayName','Acoustic sensor');
...
%         'DisplayName','Camera');
% 
%     % Create legend with all 3 handles (changed NumColumns to 3 so they sit in one row)
%     lgd_p2 = legend(legAx, [hText_leg, hA_leg, hC_leg], ...
%         'Location','south','Interpreter','latex', ...
%         'FontSize',fs_legend,'NumColumns',3,'Box','off', ...
%         'Orientation','horizontal');
% 
%     lgd_p2.Position(2) = 0.05;   % pin near bottom edge
% 
%     % % Tighten layout: leave room at bottom for the shared legend
%     % for iT = 1:nT_this
%     %     p = get(ax2_this(iT), 'Position');
%     %     ax2_this(iT).Position = [p(1), p(2)+0.10, p(3), p(4)-0.05];
%     % end
%     % Tighten layout: Shift bottom UP for legend (+0.08), 
%     % but shrink height MORE (-0.14) to leave a wide margin at the top for titles.
%     for iT = 1:nT_this
%         p = get(ax2_this(iT), 'Position');
%         ax2_this(iT).Position = [p(1), p(2)+0.09, p(3), p(4)-0.14];
%     end
% 
%     figHandles{end+1}   = fAmpFig2;
%     figBaseNames{end+1} = figName;
% 
% end % iKAfig
% 
% end % unique_ka_p2 guard
% end % idx_acou guard
...
%         strrep(sprintf('%.3f', ka_tgt), '.', 'p'), plotNormSuffix);
%     fAmpFig2 = figure('Name', figName);
%     % Reduced the width multiplier from 420 to 320 to pull subplots together
%     fAmpFig2.Position = [60, 60, 320*nT_this + 80, 480]; 
% 
%     ax2_this = gobjects(1, nT_this);
% 
%     for iT = 1:nT_this
...
%         annot_str = [main_str, bracket_str];
% 
%         % Placed above the axes via title, and shifted up to avoid the exponent
%         t_obj = title(ax_p2, annot_str, 'Interpreter','latex', 'FontSize', fs_annot, ...
%             'FontWeight','normal');
%         t_obj.Units = 'normalized';
%         t_obj.Position(2) = 1.2; % Pushes the title up by 8% to clear the multiplier
% 
%         % ---- Plot the single matching condition ----
%         ampC2  = cond_pairs_p2(ic1, 1);
%         freqC2 = cond_pairs_p2(ic1, 2);
...
%         yl = ylim(ax_p2);
% 
%         % Set y-limit with buffer
%         ylim(ax_p2, [0, max(yl(2)*1.15, hasBenchmark*1.3 + ~hasBenchmark*0.005)]);
% 
%         % Force number of ticks
%         final_yl = ylim(ax_p2);
%         ax_p2.YTick = linspace(0, final_yl(2), 6);
% 
%         % RESET formatting to default (this fixes the "Invalid numeric tick label format" error)
%         ytickformat(ax_p2, 'auto'); 
% 
%         % % Force the exponent to be placed at the top of the axis
%         % % Setting this to -3 will force the 'x 10^-3' label to appear
%         % ax_p2.YAxis.Exponent = -3;
% 
%         % Clean up axes
%         xlabel(ax_p2, '$x$ (m)', 'Interpreter', 'latex', 'FontSize', fs_label);
%         if iT == 1
...
for iKAfig = 1:length(unique_ka_p2)
    ...
    % Reduced the width multiplier from 420 to 320 to pull subplots together
    fAmpFig2.Position = [60, 60, 320*nT_this + 80, 480]; 
    t = tiledlayout(1, nT_this, 'TileSpacing', 'compact', 'Padding', 'compact');
    ...
    for iT = 1:nT_this
        ...
        % xlabel(ax_p2, '$x$ (m)', 'Interpreter', 'latex', 'FontSize', fs_label);
        % 
        % % This will now give EVERY subplot its own unique Y-axis label
        % % 1. Find the maximum data point in THIS specific subplot
        % all_y_data = [res_MeanAmp_plot(idxA2); res_MeanAmp_plot(idxC2)];
        % max_y = max(all_y_data, [], 'omitnan');
        % 
        % 
        % % 2. Set limits dynamically (e.g., 15% headroom above the max value)
        % ylim(ax_p2, [0, max_y * 1.15]);
        % 
        % % By removing the 'if/else', we ensure every subplot shows its own ticks/labels
        % % This allows you to see the unique scale for each T_set condition
        % ax_p2.FontSize = fs_axis;
        % ax_p2.LineWidth = 1.2;
        % 1. Find dynamic max and set limits
        all_y_data = [res_MeanAmp_plot(idxA2); res_MeanAmp_plot(idxC2)];
        max_y = max(all_y_data, [], 'omitnan');
   
        
        % Set limits with 15% head room
        ylim(ax_p2, [0, max_y * 1.15]);
        
        % 2. FORCE 6 TICKS
        % We use linspace to get exactly 6 values from 0 to the new limit
        final_yl = ylim(ax_p2);
        ax_p2.YTick = linspace(0, final_yl(2), 6);
        
        % 3. Y-LABEL & FORMATTING
   
        xlabel(ax_p2, '$x$ (m)', 'Interpreter', 'latex', 'FontSize', fs_label);

        % 4. LABELS: Y-label ONLY on the first subplot (iT == 1)
        if iT == 1
            ylabel(ax_p2, y_label_amp, 'Interpreter', 'latex', 'FontSize', fs_label);
        else
            ylabel(ax_p2, ''); % Ensure other subplots have no label
        end
        
        % % Force scientific notation if desired (e.g., 10^-3)
        ax_p2.YAxis.Exponent = -3; 
         ytickformat(ax_p2, '%.1f');
...
externalDrivePath = 'E:\SIWWI2026\200526\';
...
% -------------------------------------------------------
%  6b. SAVE MAIN RESULTS TABLE
%  One row per measurement (acoustic + camera), including
%  ka_meas / dka_meas (from dispersion relation, Plot 2)
%  and ka_set (from pairing metadata).
% -------------------------------------------------------
if save_results
    resultsTable = table( ...
        res_Measurement, res_File, res_Mode, res_Date, ...
        ...
        res_FFT_Freq, res_MeanFreq, ...
        res_MeanAmp, res_UncAmp, res_UncFreq, ...
        res_MeanAmp_PT, res_UncAmp_PT, ...
        res_MeanAmp_Env, res_UncAmp_Env, ...
        res_DriftRatio_water, res_RefAmp, res_NormAmp, ...
        res_ka_meas, res_dka_meas, res_ka_set, ...
        'VariableNames', { ...
        'Measurement','File','Mode','Date', ...
        'SensorID','Channel','SensorLoc_m', ...
        'InputAmp_V','InputFreq_Hz', ...
        'FFT_Freq_Hz','MeanFreq_Hz', ...
        'MeanAmp_m','UncAmp_m','UncFreq_Hz', ...
        'MeanAmp_PT_m','UncAmp_PT_m', ...
        'MeanAmp_Env_m','UncAmp_Env_m', ...
        'DriftRatio_water','RefAmp_m','NormAmp', ...
        'ka_meas','dka_meas','ka_set'});

    resultsCsvPath = fullfile(outputDir, ['Results_postprocess_' date_str '.csv']);
    writetable(resultsTable, resultsCsvPath);
    fprintf('  Results table saved: %s (%d rows)\n', resultsCsvPath, height(resultsTable));
end
