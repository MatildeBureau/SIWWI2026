    ...
    for j = 1:n_rows
        ...
        if ~has_ice(i) % water case wavelength has been derived using open-water disp rel already
            ...
            for iter_fg = 1:200
                ...
                F_val  = omega_j^2 - g * k_iter * th * (1 + alpha);
                dF_dk  = -g * th * (1 + alpha) ...
        ...
        % Append row — T_set_j is the new column
...
ax.FontSize = 10;
xlabel(ax,'$H/gT^2$','Interpreter','latex','FontSize',11);
ylabel(ax,'$2a/gT^2$', 'Interpreter','latex','FontSize',11);
...
text(ax,x_brk_lbl*1.3,y_brk_lbl*1.8,'Breaking Criterion','FontSize',8.5,'Color','k','Rotation',42,'HorizontalAlignment','center');
...
lgd = legend(ax, 'FontSize', 9, 'Box', 'on');
...
if ~isempty(output_csv_path) && ~isempty(result_rows)
    % Column headers — T_set added after T_s (measured period)
